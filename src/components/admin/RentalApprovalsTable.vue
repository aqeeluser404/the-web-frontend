          ...
          >
            ...
            >
              <template v-slot:body="props">
                ...
                <q-tr v-if="expandedUnitIds.has(props.row._id)" :props="props">
                              ...
                              <q-item-section>
                                <div v-if="sub.viewContext === 'extendedInto'">
                                  <q-badge color="teal">
                                    <template v-if="sub.historyEntry">
                                      Renewed from
                                      {{ sub.historyEntry.fromYear || "prev" }}
                                      <span
                                        v-if="
                                          sub.historyEntry.sameRoom === false
                                        "
                                        >🔄</span
                                      >
                                    </template>
    ...
    floorGroups() {
      ...
      return floorOrder
        ...
        .map((f) => {
          ...
          return { floorLevel: f, units: groups[f], roomCount, occupiedCount };
        });
    },
  },
  ...
  methods: {
      ...
      try {
        ...
        const unitRows = units.map((unit) => {
          const subUnits = unit.subUnits.map((sub) => {
            ...
            if (directMatch) {
              const hasRenewalHistory =
                directMatch.renewalHistory &&
                directMatch.renewalHistory.length > 0;
            ...
            const linkedMatch = rentalsWithUsers.find((r) => {
              ...
              const entryIndex = r.renewalHistory.findIndex(
                (h) =>
                  ...
                  (h.fromSubUnit?.roomType === identifier ||
                    h.fromSubUnit?.bedType === identifier)
              );
            ...
            if (linkedMatch) {
              let historyEntry = null;
              if (
                linkedMatch.renewalHistory &&
                linkedMatch.renewalHistory.length > 0
              ) {
    ...
    async moveToPending(row) {
      this.$q
        ...
        .onOk(async () => {
          const pendingRental = { status: "Pending" };
