    ...
    <div class="constrain-standard row justify-center q-py-md">
        ...
        >
          ...
          <q-card-section>
            <div>
              <q-item class="row justify-between">
                <q-radio
                  :disable="isEditingDisabled"
                  style="width: 48%"
                  v-model="userDetails.studentInfo.isRegisteredStudent"
                  :val="true"
            ...
            <div>
              <q-item class="row justify-between">
                <q-radio
                  :disable="isEditingDisabled"
                  style="width: 48%"
                  v-model="userDetails.studentInfo.hasBursary"
                  :val="true"
            ...
            <div v-if="userDetails.studentInfo.isRegisteredStudent === true">
              <q-item>
                <q-item-section class="text-left text-subtitle1"
                  >Student Number</q-item-section
                >
                <q-item-section class="text-left">
                  <q-input
                    :disable="isEditingDisabled"
                    v-model="userDetails.studentInfo.studentNumber"
                  />
                </q-item-section>
              </q-item>
              <q-item>
                <q-item-section class="text-left text-subtitle1"
                  >Registered Institution</q-item-section
                >
                <q-item-section class="text-left">
                  <q-input
                    :disable="isEditingDisabled"
                    v-model="userDetails.studentInfo.registeredInstitution"
                  />
                </q-item-section>
              </q-item>
            </div>
          ...
          <q-card-section>
            <q-item>
              <q-item-section class="text-left text-subtitle1"
                >Guardian Name</q-item-section
              >
              <q-item-section class="text-left">
                <q-input
                  :disable="isEditingDisabled"
                  v-model="userDetails.guardianName"
            ...
            <q-item>
              ...
              >
                <span>
                  ...
                  <span
                    ...
                    v-if="
                      userDetails &&
                      userDetails.guardianVerification &&
                      userDetails.guardianVerification.isVerified
                    "
                  >
                    <q-icon
                      color="secondary"
                  ...
                  <span
                    class="text-subtitle1 q-ml-md"
                    v-else-if="userDetails && userDetails.guardianEmail"
                  >
                    <q-icon color="negative" name="eva-alert-circle-outline" />
                  </span>
                </span>
              ...
              <q-item-section class="text-left">
                <div class="row items-center q-gap-sm">
                  <q-input
                    :disable="isEditingDisabled"
                    v-model="userDetails.guardianEmail"
                    type="email"
      ...
      <div v-if="!loading" class="col-md-6 col-12 full-height">
        <q-card class="full-height soft-shadow-card">
          ...
          <q-card-section>
            <div class="q-mb-sm">
              Please verify that all provided information is accurate before
              proceeding. Kindly ensure the following documents are uploaded:
            </div>
            <!-- <ul>
            ...
            <div
              class="q-mb-md"
              :key="
                categoryLock.lockedCategory
                  ? 'locked-' + categoryLock.lockedCategory
                  : 'unlocked'
              "
            >
            ...
            >
              ...
              <div
                v-for="docType in rentalDocumentTypes"
                :key="docType.type"
                class="cursor-pointer q-py-sm q-px-md"
                style="font-weight: 500"
                @click="handleRentalDocumentClick(docType.type)"
              >
                <q-icon
                  class="q-mr-sm"
                  :color="
                    hasRentalDocument(docType.type) ? 'secondary' : 'negative'
                  "
                  :name="
                    hasRentalDocument(docType.type)
            ...
            <br />Once your rental application has been submitted; <br />
            No further changes to your
            <span style="text-decoration: underline">email</span> or
            <span style="text-decoration: underline">sensitive data</span> will
            be permitted unless the application has been rejected or ended.
            <br /><br /><b>Note:</b>
          ...
          <q-card-section>
            <div class="q-mb-sm">
              Use this key to grant family members or dependents access to
              shared accommodation within a designated unit.
            </div>
            <ul>
              ...
              <li v-else>
                You have not utilized an access or reference key in your current
                application.
              </li>
            </ul>
          </q-card-section>
...
export default {
  data() {
    return {
      ...
      documentCategories: [
        {
          ...
          documents: [
            ...
            {
              type: "private_student_registration",
              label: "Student Registration Form",
            },
            {
              type: "private_id_student",
              label: "Identity Documents - Student responsible",
            },
            {
              type: "private_id_person",
              label: "Identity Documents - Person responsible",
            },
            { type: "private_proof_of_address", label: "Proof of Address" },
            {
              type: "private_3_months_payslips",
              label: "3 months latest Payslips",
            },
            {
              type: "private_3_months_bank_statements",
              label: "3 months Bank statements",
            },
            {
              type: "private_credit_check_proof_of_payment",
              label: "Credit Check Proof of Payment",
            },
          ],
        },
        {
          ...
          documents: [
            ...
            {
              type: "business_student_registration",
              label: "Student Registration Form",
            },
            {
              type: "business_id_directors",
              label: "Identity Documents of all Directors",
            },
            { type: "business_proof_of_address", label: "Proof of Address" },
            { type: "business_cipc_documents", label: "CIPC Documents" },
            {
              type: "business_6_months_bank_statements",
              label: "6 Months Bank statements",
            },
            {
              type: "business_credit_check_proof_of_payment",
              label: "Credit Check Proof of Payment",
            },
          ],
        },
        {
          ...
          documents: [
            ...
            {
              type: "bursary_student_registration",
              label: "Student Registration Form",
            },
            ...
            {
              type: "bursary_credit_check_proof_of_payment",
              label: "Credit Check Proof of Payment",
            },
          ],
        },
      ...
      rentalDocumentTypes: [
        {
          type: "Signed And Filled Application Form",
          label: "Credit Check Application",
        },
        {
          type: "Signed And Filled Lease Form",
  ...
  computed: {
    ...
    verifyButtonLabel() {
      const emailUnverified =
        this.userDetails?.verification?.isVerified === false;
      const guardianUnverified =
        this.userDetails?.guardianEmail &&
        this.userDetails?.guardianVerification?.isVerified !== true;

      if (emailUnverified && guardianUnverified)
        return "Verify Email & Guardian";
    ...
    displayedCategory() {
      return (
        this.categoryLock.lockedCategory || this.categoryLock.selectedCategory
      );
    },
    displayedCategoryObj() {
      return (
        this.documentCategories.find(
          (c) => c.category === this.displayedCategory,
        ) || { documents: [] }
      );
    },
    ...
    hasPendingRental() {
      if (this.myRentals && Array.isArray(this.myRentals)) {
        const pendingRentals = this.myRentals.filter(
    ...
    hasCreditCheckApplication() {
      ...
      return pendingRental.documents.some(
        (doc) =>
  ...
  methods: {
    ...
    async sendOutCreditCheckApplication() {
      ...
      if (!this.userDetails?.guardianName || !this.userDetails?.guardianEmail) {
        this.$q.notify({
          ...
          position: "top",
          message:
            "Please fill in your guardian information to receive your credit check application!",
        });
        return;
      }
    ...
    async cleanupCreditCheckStorage() {
      ...
      const creditCheckKeys = keys.filter((key) =>
        key.startsWith("sentCreditCheckEmail_"),
      );

      for (const key of creditCheckKeys) {
        const rentalId = key.replace("sentCreditCheckEmail_", "");

        const rentalExists = this.myRentals?.some(
          (rental) => rental._id === rentalId,
        );
        const isPending = this.myRentals?.some(
          (rental) => rental._id === rentalId && rental.status === "Pending",
        );
    ...
    async removeAllDocuments() {
      if (this.userDetails.documents.length > 0) {
        this.$q
          ...
          .onOk(async () => {
            const response = await UserService.clearAllUserDocs(
    ...
    async handleDialogClose() {
      ...
      await this.fetchUserDetails();
      if (
        !this.categoryLock.lockedCategory &&
        this.userDetails.documents.length > 0
      ) {
    ...
    async deleteDocument(fileId) {
      const response = await UserService.removeUserDoc(
        this.userDetails._id,
        fileId,
      );
      if (response) {
    ...
    async checkEditingDisabled() {
      ...
      //   return;
      // }
      // const userRentals = await RentalService.findMyRentals(
      //   this.userDetails._id,
      // );
      // const activeOrPendingRentals = userRentals.filter((rental) =>
      //   ["Pending", "Active"].includes(rental.status),
      // );
      // this.isEditingDisabled = activeOrPendingRentals.length > 0;

      return (this.isEditingDisabled = false);
    },
    validateFields() {
    ...
    updatedStudentStatus() {
      ...
      if (isRegistered === true) {
        return {
          ...
          registeredInstitution:
            this.userDetails.studentInfo.registeredInstitution,
    ...
    async fetchUserDetails() {
      ...
      if (this.userDetails.dateOfBirth) {
        this.userDetails.dateOfBirth = this.formatDateForDisplay(
      ...
      this.currentAccessKey = response.find(
        (rental) =>
