<template>
  <q-page class="bg-grey-3">
    <div v-if="!loading">
      ...
      >
        <q-card class="col-md-4 col-12 stats-card full-height">
          ...
          <q-card-section>
            <div class="row q-gutter-sm justify-center">
              <q-btn
                ...
                :key="option.value"
                :color="
                  callLog.callType === option.value ? 'primary' : 'grey-8'
                "
                :icon="option.icon"
                flat
                rounded
          ...
          <q-card
            class="q-pa-lg shadow-2 rounded-borders full-width flex flex-center"
          >
            <q-card-section
          ...
          <q-card-section class="q-px-none">
            ...
            <div class="row q-gutter-md">
              ...
              <q-select
                ...
                :options="[
                  'All',
                  ...callTypeOptions.map((option) => option.label),
                ]"
                label="Call Type"
                @update:model-value="filterCallLogs"
            ...
            >
              ...
              <template v-slot:body-cell-vendorType="props">
                <q-td :props="props">
                  <div
                    v-if="
                      props.row.vendorInfo && props.row.vendorInfo.vendorType
                    "
                  >
            ...
            <q-card-section>
              <q-item>
                <q-item-section class="text-left text-subtitle1"
                  >Please describe your issue</q-item-section
                >
                <q-item-section>
                  <q-input
          ...
          <q-card-section class="row justify-center">
            <q-item>
              <q-item-section class="text-subtitle1"
                >You’ll have access to the call log system once your rental
    ...
    <q-dialog v-model="callLogDialog">
...
export default {
  ...
  methods: {
    ...
    async initiateCall() {
      ...
      if (
        !Array.isArray(this.callLog.images) ||
        this.callLog.images.length === 0
      ) {
        this.$q.notify({
          type: "negative",
        ...
        .onOk(async () => {
          ...
          if (response) {
            this.$q.notify({
              type: "positive",
              color: "primary",
              message: "Your call log has been sent!",
            });
            ...
            this.$q.notify({
              type: "negative",
              message: "Call log failed. Please try again.",
            });
          }
        })
        .onCancel(() => {});
    ...
    filterCallLogs() {
      this.filteredCallLogs = this.myCallLogs.filter((log) => {
        ...
        const typeMatch =
          ...
          this.selectedCallType === "All" ||
          log.callType === this.selectedCallType;

        // Exception: Allow filtering by call types starting with "Other - "
        const otherTypeMatch =
          this.selectedCallType === "Other" &&
          log.callType.startsWith("Other - ");
    ...
    viewCalllogUpdates(evt, row) {
      this.selectedCallLog = row;
