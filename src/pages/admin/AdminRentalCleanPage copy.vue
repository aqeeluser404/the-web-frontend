  ...
  methods: {
    async findAllRentals() {
      ...
      try {
        ...
        const activeRentals = response.filter(rental => rental.status === "Active");
        ...
        // Sorting rentals by unitType with proper numeric + letter ordering
        rentalsWithUsers.sort((a, b) => {
          const parseName = (name) => {
            if (!name) return [0, 0, 0, 0, ""];
            const parts = name.trim().split(" ");
            const sequence = parts.length > 1 ? parts[1] : parts[0];

            // Match 3 or 4 numeric groups, plus optional trailing letter
            // Examples: "1-01-01", "1-01-02", "1-02-01", "1-01-01A"
        ...
        // Apply tinted property
        let lastPrefix = null;
        let toggle = false;

        rentalsWithUsers.forEach(r => {
        ...
        this.filteredRentals = rentalsWithUsers;
