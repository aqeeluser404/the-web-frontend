<template>
  <q-page class="bg-grey-3">
    ...
    <div
      class="constrain-standard row justify-center q-pb-md"
      v-show="!loading"
    >
  ...
  methods: {
    ...
    async downloadData() {
      this.$q
        .dialog({
          title: "Download Data",
          message:
            "You are about to export all rental history data. Would you like to proceed?",
          color: "primary",
          cancel: true,
          persistent: true,
        })
        .onOk(() => {
          try {
            ...
            const rows = this.filteredRentals.map((rental, index) => ({
              ...
              "Applicant Contact": (rental.userPhone || "Unassigned").replace(
                /,/g,
                ""
              ),
    ...
    async findAllRentals() {
      ...
      try {
        ...
        if (!this.selectedHistoryYear && this.availableYears.length) {
    ...
    filteredByRentalStatus() {
      // 1. Year filter first — independent, always applied
      let base = this.selectedHistoryYear
        ? this.currentRentals.filter(
            (r) => Number(r.unitYear) === Number(this.selectedHistoryYear)
          )
        : this.currentRentals;

      // 2. Status filter applies WITHIN that year's subset
      if (this.selectedRentalStatus && this.selectedRentalStatus !== "All") {
        base = base.filter((r) => r.status === this.selectedRentalStatus);
      }
    ...
    async deleteRental(rental) {
      this.$q
        .dialog({
          title: "Confirm",
          message:
            "You are about to delete this rental application. This action is irreversible. Continue?",
          color: "primary",
          cancel: true,
          persistent: true,
        })
    ...
    async endRental(rental) {
      if (rental.status !== "Active") {
        this.$q.notify({
          type: "negative",
          message: "You can only end an active rental.",
        });
        return;
      }
      ...
      this.$q
        .dialog({
          title: "Confirm",
          message:
            "You are about to terminate this rental before the agreement end date. Continue?",
          color: "primary",
          cancel: true,
          persistent: true,
        })
